...
#include <factories/product_individual_factory.hpp>
#include <factories/task_individual_factory.hpp>
...
void UsageScene1() {
  ...
  auto factory = std::make_shared<TaskIndividualFactory>();
  auto selection = std::make_shared<TournamentSelection>(factory);
  auto crossover = std::make_shared<TwoPointCrossover>(factory);
  auto mutate = std::make_shared<RandomReplacementMutate>(factory);
  const int max_iterations = 10000;
  GeneticAlgorithmSolver solver(factory, selection, crossover, mutate, max_iterations);
...
void UsageScene2() {
  ...
  auto factory = std::make_shared<ProductIndividualFactory>();
  auto selection = std::make_shared<RankSelection>(factory);
  auto crossover = std::make_shared<SinglePointCrossover>(factory);
  auto mutate = std::make_shared<RandomReplacementMutate>(factory);
  const int max_iterations = 10000;
  GeneticAlgorithmSolver solver(factory, selection, crossover, mutate, max_iterations);

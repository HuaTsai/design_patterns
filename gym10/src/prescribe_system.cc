...
Prescription PrescribeSystem::Prescribe(const std::string& name, const std::vector<Symptom> &symptoms) {
...
void PrescribeSystem::SaveCaseToDatabase(const std::string& filepath, const std::string& name, const Case &c) {
  auto &patient = database_.GetPatientByName(name);
  patient.AddCase(c);
